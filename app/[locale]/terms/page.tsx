    ...
    <main className="min-h-screen bg-[#f7f6f2] px-5 py-8">
      <article className="mx-auto max-w-3xl rounded-lg border border-[var(--line)] bg-white p-6">
        ...
        <div className="mt-6 grid gap-6 text-sm leading-7 text-[var(--muted)]">
          <section>
            <h2 className="text-lg font-semibold text-[var(--foreground)]">{t('serviceTitle')}</h2>
            <p className="mt-2">{t('serviceCopy')}</p>
          </section>
          <section>
            <h2 className="text-lg font-semibold text-[var(--foreground)]">{t('accountTitle')}</h2>
            <p className="mt-2">{t('accountCopy')}</p>
          </section>
          <section>
            <h2 className="text-lg font-semibold text-[var(--foreground)]">{t('billingTitle')}</h2>
            <p className="mt-2">{t('billingCopy')}</p>
          </section>
          <section>
            <h2 className="text-lg font-semibold text-[var(--foreground)]">{t('availabilityTitle')}</h2>
            <p className="mt-2">{t('availabilityCopy')}</p>
          </section>
          <section>
